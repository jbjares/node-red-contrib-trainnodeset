...
module.exports = function(RED) {
    ...
    function getProperty(node,msg,done) {
        if (node.propertyType === 'jsonata') {
            RED.util.evaluateJSONataExpression(node.property,msg,(err,value) => {
                if (err) {
                    done(RED._("Holder.errors.invalid-expr",{error:err.message}));
                } else {
    ...
    function HolderNode(config) {
        console.log("Starting Holder Node");
        ...
        this.artifactsNumber = config.artifactsNumber;
        var count = 0;
        ...
        if (this.propertyType === 'jsonata') {
            try {
                ...
                this.error(RED._("Holder.errors.invalid-expr",{error:err.message}));
                return;
            }
        }
        ...
        function addMessageToGroup(id, msg, parts) {
            ...
            if ((max_msgs > 0) && (pendingCount > max_msgs)) {
                clearPending();
                node.error(RED._("Holder.errors.too-many"), msg);
            }
    ...
    RED.nodes.registerType("Holder", HolderNode);
}